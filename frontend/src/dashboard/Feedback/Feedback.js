import React from 'react'
import { Link } from 'react-router-dom/cjs/react-router-dom'

const Feedback= () => {
  return (
    <div>
        <nav className="navcls" >
        <img src="data:image/png;base64," className="logo"></img>
          <div className="navdiv1">
          <Link to="/home" className="linkcls" >Home </Link>
          <Link to="/journel"  className="linkcls" >Journel </Link>
          <Link to="/start"  className="linkcls" >Start-ups </Link>
          <Link to="/ipr"  className="linkcls" >IPr management</Link>
          <Link to="/feed" className="linkcls">Feedback</Link>
          </div>
          <div className="navdiv2">
            <Link to="/profile"  className="linkcls">Profile </Link>
          </div>
        </nav>
        <div >
    <div className='fee'>
        <div className='feedback'>
        <form>
            <label>First Name:</label>
            <input type='text'></input>
            <br></br><br/>
            <label>Last Name:</label>
            <input type='text'></input>
            <br/><br/>
            <label>Please provide feedback on the quality of our service</label><br/>
            <div >
            <input type='radio' name='feedback' id='1' ></input>
            <label>Excellent</label> 
            <input type='radio' name='feedback' id='2'></input> 
            <label>Very Good</label>
            <input type='radio' name='feedback' id='3'></input> 
            <label>Good</label>
            <input type='radio' name='feedback' id='4'></input> 
            <label>Average</label>
            <input type='radio' name='feedback' id='5'></input> 
            <label>Poor</label>
            
            <br/><br/>
            </div>
            <label>How often do you use our app ?</label><br/>
            <input type='text' placeholder='Everyday/Once a week/bi-weekly' style={{width: "100%"}}></input><br/><br/>
            <label>What is the motivation to use our app?</label><br/>
            <textarea placeholder='what problem does  it solve you'></textarea><br/><br/>
            <label>What would you like to see improved the most</label><br/>
            <input type='text' style={{width: "100%"}}></input><br/><br/>
            <label>What kind of suggestion you have?</label>
            <br/><textarea/>
        </form>
        <button type='submit'>submit</button>
        </div>
    </div>
    </div>
    </div>
  )
}

export default Feedback
