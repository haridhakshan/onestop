...
const View = () => {
    const {id} = useParams();
    const {data: journels , error , isPending} = Reuse("http://localhost:8000/Journels/" + id)
    const history = useHistory();
    const handleClick = () => {
        fetch("http://localhost:8000/Journels/" + journels.id,{
            method: 'DELETE'
        }).then(()=>{
            console.log("Journel deleted");
        });
        history.goBack();
    ...
    return (
      ...
      <div className='view'>
          ...
          {journels &&(
            <article >
                <h1>{journels.title}</h1>
                <p>written by {journels.name}</p>
                <br></br><br></br>
                <div>{journels.content} <br/><br/></div>
