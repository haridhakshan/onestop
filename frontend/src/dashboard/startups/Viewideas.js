import React from 'react'
import { Link, useHistory, useParams } from 'react-router-dom/cjs/react-router-dom';
import Reuse from '../Reuse';


const Viewideas = () => {
    const {id} = useParams();
    const {data: ideas , error , isPending} = Reuse("http://localhost:8000/Idea/" + id)
    const history = useHistory();
    const handleClick = () => {
        fetch("http://localhost:8000/Idea/" + ideas.id,{
            method: 'DELETE'
        }).then(()=>{
            console.log("idea deleted");
        });
        history.goBack();
        
    }
    return (

    <div>
        <nav className="navcls" >
        <img src="data:image/png;base64," className="logo"></img>
          <div className="navdiv1">
          <Link to="/home" className="linkcls" >Home </Link>
          <Link to="/journel"  className="linkcls" >Journel </Link>
          <Link to="/start"  className="linkcls" >Start-ups </Link>
          <Link to="/bingo"  className="linkcls" >IPR MANAGEMENT</Link>
          </div>
          <div className="navdiv2">
            <Link to="/profile" style={{color:'white',textDecerator:'none'}}>Profile</Link>
          </div>
        </nav>
        <div className='super'>
      <div className='view'>
          {isPending && <div>Loading...</div>}
          
          {ideas &&(
            <article >
                <h1>{ideas.Title}</h1>
                <p>written by {ideas.Name}</p>
                <br></br><br></br>
                <div>{ideas.overview}</div>
                <br></br><br></br>
                <div>{ideas.body}</div>
                
                <button onClick={handleClick}>delete</button>
            </article>
          )}
      </div>
      </div>
      </div>
    )
}

export default Viewideas